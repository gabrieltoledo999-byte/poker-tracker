...
export function PlayerSeat(props: {
  ...
  return (
    <button
      ...
      className={`w-[172px] overflow-visible rounded-2xl border px-3 py-2.5 text-left transition ${tone} ${props.isSelected ? "ring-2 ring-cyan-400/40" : ""} ${props.isActing ? "scale-[1.02] shadow-[0_0_24px_rgba(34,211,238,0.25)]" : ""}`}
    >
      ...
      <p className="truncate text-base font-bold">{seat.name}</p>
      <div className="mt-1 inline-flex max-w-full items-center rounded-md border border-cyan-200/45 bg-cyan-300/16 px-2 py-0.5 shadow-[0_0_14px_rgba(34,211,238,0.22)]">
        <p className="truncate text-sm font-extrabold tracking-wide text-cyan-50">{formatValue(seat.stackApprox, props.displayUnit, props.bigBlind)}</p>
      </div>
